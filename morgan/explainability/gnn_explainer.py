...
class GNNExplainer(torch.nn.Module):
    ...
    def explain_node(
        self, node_idx: int, data: Data, **kwargs
    ) -> Tuple[Tensor, Tensor]:
        """
        ...
        data_sub.edge_type = data.edge_type[
            hard_edge_mask
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            loss = self.__loss__(
                mapping.item() if torch.is_tensor(mapping) else mapping,
                log_logits,
                pred_label,
            )
            loss.backward()
    ...
    def explain_graph_s2v(self, dataset, param, idd):
        ...
        return self.node_feat_mask.view(
            -1, 1
        ).detach()  # self.edge_mask.detach().sigmoid()
    ...
    def plot_graph(
        ...
        for index, edge in enumerate(edge_mask):
            if edge:
                edge_list.append(
                    (edge_index[0, index].cpu(), edge_index[1, index].cpu())
                )
        ...
        data = Data(
            edge_index=edge_index.cpu(), att=edge_mask, y=y, num_nodes=y.size(0)
        ).to("cpu")
        ...
        nx.draw_networkx_edges(
            G, pos, width=3, alpha=0.5, edge_color=color, edge_cmap=plt.cm.Reds
        )
