...
def parse_args():
    ...
    parser.add_argument(
        "--dataset_name", default="CPDB_cdgps", type=str, help="the name of dataset"
    )
...
def main(args):
    ...
    if dataset_name in ["CPDB", "IRefIndex_2015", "IRefIndex", "PCNet", "STRINGdb"]:
        ...
        "CPDB_cdgps",
        "IRefIndex_2015_cdgps",
        "IRefIndex_cdgps",
        "PCNet_cdgps",
        "STRINGdb_cdgps",
        "CPDB_cdgps_shuffled",
    ]:
        multidim_graph = load_processed_graph(
        ...
        multidim_graph = load_processed_graph(
            f"data/components/multidim_graph/5d/{dataset_name}_multiomics.pt"
        )
        num_features = multidim_graph.x.shape[1]
        num_classes = multidim_graph.y.max().item() + 1
        ...
        multidim_graph = load_processed_graph(
            f"data/components/multidim_graph/4d/{dataset_name}_multiomics.pt"
        )
        num_features = multidim_graph.x.shape[1]
        num_classes = multidim_graph.y.max().item() + 1
        ...
        multidim_graph = load_processed_graph(
            f"data/components/multidim_graph/3d/{dataset_name}_multiomics.pt"
        )
        num_features = multidim_graph.x.shape[1]
        num_classes = multidim_graph.y.max().item() + 1
        ...
        multidim_graph = load_processed_graph(
            f"data/components/multidim_graph/2d/{dataset_name}_multiomics.pt"
        )
        num_features = multidim_graph.x.shape[1]
        num_classes = multidim_graph.y.max().item() + 1

    print(
        f"Loaded multi-relational graph with {multidim_graph.num_nodes} nodes and {multidim_graph.edge_index.size(1)} edges"
    )
    ...
    print(
        f"Split multi-relational graph into {len(relation_graphs)} relation-specific graphs."
    )
    ...
    for i, seed in enumerate(seeds):
        ...
        for epoch in range(1, args["epochs"] + 1):
            ...
            epoch_end = time.time()
            print(
                f"Run {i} - Epoch {epoch} runtime: {epoch_end - epoch_start:.2f} seconds"
            )
        ...
        print(
            f"Recall: {test_recall:.4f}, Precision: {test_precision:.4f}, F1: {test_f1:.4f}\n"
        )
