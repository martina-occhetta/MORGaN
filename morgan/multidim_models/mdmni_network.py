...
class generate_graph(object):
    ...
    def generate_graph(
        ...
        ppi = pd.read_csv(
            os.path.join("Data/multi-view_network/CPDB_ppi.tsv"),
            sep="\t",
            compression="gzip",
            encoding="utf8",
            usecols=["partner1", "partner2"],
        )
        ...
        ppi_cpdb = ppi_cpdb.applymap(lambda x: 0 if x < thr_cpdb else 1).astype(
            int
        )  # Convert the scores to 0 or 1 based on a threshold value
        print(f"The shape of CDPB network_adj：", ppi_cpdb.shape)
        ppi_cpdb.to_csv(
            os.path.join(
                self.graph_path, str(file_name) + "_cpdb_" + str(thr_cpdb) + ".csv"
            )
        )
        ...
        ppi_string = pd.DataFrame(
            ppi_adj_matrix, index=final_gene_node, columns=final_gene_node
        )
        print(f"The shape of String network_adj：", ppi_string.shape)
        ppi_string.to_csv(
            os.path.join(
                self.graph_path, str(file_name) + "_string_" + str(thr_string) + ".csv"
            )
        )
        ...
        domain_df = pd.DataFrame(
            domain_matrix, index=final_gene_node, columns=final_gene_node
        )
        print(f"The shape of Domain network_adj：", domain_df.shape)
        domain_df.to_csv(
            os.path.join(
                self.graph_path, str(file_name) + "_domain_" + str(thr_domain) + ".csv"
            )
        )
        ...
        exp = pd.read_csv(
            os.path.join("Data/multi-view_network/Expression_similarity.csv"),
            sep="\t",
            index_col=0,
        )
        ...
        go = pd.read_csv(
            "Data/multi-view_network/Semantic_similarity.csv", sep="\t", index_col=0
        )
        go_matrix = go.applymap(lambda x: 0 if x < thr_go else 1).astype(int)
        np.fill_diagonal(go_matrix.values, 0)

        path = pd.read_csv(
            os.path.join("Data/multi-view_network/Pathway_similarity.csv"),
            sep="\t",
            index_col=0,
        )
        ...
        seq = pd.read_csv(
            os.path.join("Data/multi-view_network/Sequence_similarity.csv"),
            sep="\t",
            index_col=0,
        )
        ...
        for matrix in [exp_matrix, go_matrix, path_matrix, seq_matrix]:
            temp = pd.DataFrame(
                index=final_gene_node, columns=final_gene_node
            )  # Create a temporary data frame called temp
        ...
        networklist[0].to_csv(
            os.path.join(
                self.graph_path, str(file_name) + "_exp_" + str(thr_exp) + ".csv"
            )
        )
        networklist[1].to_csv(
            os.path.join(
                self.graph_path, str(file_name) + "_go_" + str(thr_go) + ".csv"
            )
        )
        networklist[2].to_csv(
            os.path.join(
                self.graph_path, str(file_name) + "_path_" + str(thr_path) + ".csv"
            )
        )
        networklist[3].to_csv(
            os.path.join(
                self.graph_path, str(file_name) + "_seq_" + str(thr_seq) + ".csv"
            )
        )
