...
def run_experiment(config: Dict[str, Any], dataset_name: str):
    ...
    try:
        ...
        if exit_code != 0:
            logging.error(f"Experiment failed for dataset {dataset_name} with exit code {exit_code}")
        else:
            logging.info(f"Successfully completed experiment for dataset {dataset_name}")
...
def main():
    parser = argparse.ArgumentParser(description='Run experiments from YAML configuration')
    ...
    try:
        ...
        experiment_dir = os.path.join('data', 'paper', 'graphs', config['experiment_type'])
