...
def linear_probing_for_transductive_node_classifcation(model, graph, feat, optimizer, max_epoch, device, mute=False):
    ...
    train_mask = graph.train_mask.squeeze() if hasattr(graph, 'train_mask') and len(graph.train_mask.shape) > 1 else graph.train_mask
    val_mask = graph.val_mask.squeeze() if hasattr(graph, 'val_mask') and len(graph.val_mask.shape) > 1 else graph.val_mask
    test_mask = graph.test_mask.squeeze() if hasattr(graph, 'test_mask') and len(graph.test_mask.shape) > 1 else graph.test_mask
    labels = graph.y.squeeze() if hasattr(graph, 'y') and len(graph.y.shape) > 1 else graph.y

    # Ensure all tensors exist
    if not all(hasattr(graph, attr) for attr in ['train_mask', 'val_mask', 'test_mask', 'y']):
        raise ValueError("Graph missing required attributes (train_mask, val_mask, test_mask, or y)")
