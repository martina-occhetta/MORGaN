...
def load_processed_multidim_graph(filepath: str) -> Data:
    ...
    if not isinstance(data, HeteroData):
        raise ValueError(
            "Loaded object is not a torch_geometric.data.HeteroDatas object."
        )
    return data
...
def load_h5_graph(PATH, LABEL_PATH, ppi):
    ...
    label_df = pd.read_csv(LABEL_PATH, sep="\t").astype(
        str
    )  # TODO fix this for druggable gene prediction
...
def load_mutag_dataset():
    ...
    data.num_edge_types = max(
        4, data.edge_type.max().item() + 1
    )  # At least 4 for MUTAG
...
def load_dataset(dataset_name):
    ...
    if dataset_name == "custom_synthetic":
        ...
        graph = train_val_test_split(
            graph, train_ratio=0.7, val_ratio=0.15, test_ratio=0.15
        )
        ...
        "CPDB_cdgps_CNA_GE_METH",
        "CPDB_cdgps_CNA_GE_MF",
        "CPDB_cdgps_CNA_METH_MF",
        "CPDB_cdgps_CNA",
        "CPDB_cdgps_GE_METH_MF",
        "CPDB_cdgps_GE",
        "CPDB_cdgps_METH",
        "CPDB_cdgps_MF",
        "CPDB_cdgps_random_features",
    ]:
        graph = load_processed_graph(
            f"data/components/multidim_graph/6d/feature_ablations/{dataset_name}.pt"
        )
        num_features = graph.x.shape[1]
        num_classes = graph.y.max().item() + 1
