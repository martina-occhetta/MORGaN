...
def plot_average_PR_curve(pred_all, label_all, fig_save_path):
    ...
    for i in range(len(pred_all)):
        ...
        aupr = auc(rec, pr)
        plt.plot(rec, pr, lw=4, alpha=0.4, label="Fold %d (AUPR = %0.2f)" % (i, aupr))
    ...
    pr_upper = np.append(pr_upper, 1.0)
    pr_lower = np.append(pr_lower, 1.0)
    mean_rec = np.append(mean_rec, 0.0)
...
def plot_average_ROC_curve(pred_all, label_all, fig_save_path):
    ...
    for i in range(len(pred_all)):
        ...
        aucs.append(auroc)
        plt.plot(
            fpr, tpr, lw=4, alpha=0.3, label="Fold %d (AUROC = %0.2f)" % (i, auroc)
        )
...
class EarlyStopping:
    ...
    def __call__(self, val_loss, model):
        ...
        if self.best_score is None:
            ...
            self.best_model = model.state_dict()  # Save initial model state
            ...
            self.best_score = score
            self.best_model = model.state_dict()  # Update best state
            self.save_checkpoint(val_loss, model)
            self.counter = 0
